SELECT 	BRN."Region" AS "RegionId",
	...
	COUNT(DISTINCT 
		CASE 
			WHEN CSE."CaseStatusId" IN (1,2,3) THEN ACC."AccountId"
			ELSE NULL 
		END) AS "CurrentAccounts", 
	COUNT(DISTINCT 
		CASE 
			WHEN CSE."CaseStatusId" IN (1,2,3) THEN CSE."DebtorId"
			ELSE NULL 
		END) AS "CurrentClients" , 
	COUNT(DISTINCT 
		CASE 
			WHEN CSE."CaseStatusId" IN (1,2,3) THEN CSE."CaseId"
			ELSE NULL 
		END) AS "CurrentCases",
	...
	NULLIF(COUNT(DISTINCT 
		CASE 
			WHEN CSE."WorkableCase" = false IS NULL THEN CSE."CaseId"
			ELSE NULL
...
GROUP BY BRN."Region", RGN."Description", SBC."CategoryId", CTG."Description", SBC."SubCategoryId",	SBC."Description"
