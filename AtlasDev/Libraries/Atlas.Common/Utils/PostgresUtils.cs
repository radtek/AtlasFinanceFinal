...
namespace Atlas.Data.Utils
{
  ...
  {
    /// <summary>
    /// Creates a safe SQL textual expression of a given object, with unicode- for ASS. Only supports common types.
    /// </summary>   
    ...
    public static string PSQLStringyfy(object value, string fieldName = "")
    {
      ...
      try
      {
        ...
        else if (valueType == typeof(Decimal))
        {
          ...
          return decimalVal.Value.ToString("F4", CultureInfo.InvariantCulture);
        }

        else if (valueType == typeof(float))
        {
          var floatVal = value as float?;
          return floatVal.Value.ToString("F4", CultureInfo.InvariantCulture);
        }
