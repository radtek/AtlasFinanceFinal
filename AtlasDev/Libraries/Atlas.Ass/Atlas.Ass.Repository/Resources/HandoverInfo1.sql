...
FROM (
  ...
  GROUP BY brnum, hovrdate, CASE WHEN payno > 6 AND payno < 12 THEN 6 ELSE payno END) HO
 LEFT JOIN (
    SELECT 1 as payno, brnum, LN.hovrdate AS "HandoverDate", COUNT(DISTINCT CONCAT(client,brnum)) AS "ClientQuantity"
    FROM company.loans LN 
    WHERE LN.hovrdate BETWEEN '{0}' AND '{1}' 
      AND LN.brnum IN ('{2}')
    GROUP BY 1, 2, 3) CH ON HO."PayNo" = CH.payno AND HO."LegacyBranchNumber" = CH.brnum AND HO."HandoverDate" = CH."HandoverDate"