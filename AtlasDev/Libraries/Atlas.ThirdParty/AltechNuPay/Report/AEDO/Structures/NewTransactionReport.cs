...
{
    public class NewTransactionReport
    {
        #region public Fields

        public AEDOReportNewTransactionDTO aedoReport;

        #endregion

        public NewTransactionReport(XElement item)
        {
            aedoReport = new AEDOReportNewTransactionDTO();

            aedoReport.ReportNewTransaction.TransactionId = Int64.Parse(item.Element("transaction_id").Value);
            aedoReport.ReportNewTransaction.ContractRef = item.Element("contract_ref").Value;
            aedoReport.ReportNewTransaction.ServiceType = item.Element("service_type").Value;
            aedoReport.ReportNewTransaction.ValueDT = DateTime.ParseExact(item.Element("value_date").Value, "yyyyMMdd", CultureInfo.InvariantCulture);
            aedoReport.ReportNewTransaction.SubmitDT = DateTime.ParseExact(item.Element("submit_date").Value + " " + item.Element("submit_time").Value, "yyyyMMdd HH:mm:ss", CultureInfo.InvariantCulture);
            aedoReport.StartDT = DateTime.ParseExact(item.Element("start_date").Value, "yyyyMMdd", CultureInfo.InvariantCulture);
            aedoReport.LastSubmissionDT = !string.IsNullOrEmpty(item.Element("last_submission_date").Value) ? DateTime.ParseExact(item.Element("last_submission_date").Value + " " + item.Element("last_submission_time").Value, "yyyyMMdd HH:mm:ss", CultureInfo.InvariantCulture) : (DateTime?)null;
            ...
            aedoReport.InstalmentAmount = FileStructureHelper.ConvertStringToDecimalPoint(item.Element("instalment_amount").Value);
