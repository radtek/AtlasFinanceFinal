...
namespace Atlas.ThirdParty.AltechNuPay.Report.AEDO
{
    public class RetryReport
    {
        ...
        public AEDOReportRetryDTO aedoReport;

        #endregion

        public RetryReport(XElement item)
        {
            aedoReport = new AEDOReportRetryDTO();

            aedoReport.ReportRetry.TransactionId = Int64.Parse(item.Element("transaction_id").Value);
            aedoReport.ReportRetry.ContractRef = item.Element("contract_ref").Value;
            aedoReport.ReportRetry.ServiceType = item.Element("service_type").Value;
            aedoReport.ReportRetry.ValueDT = DateTime.ParseExact(item.Element("value_date").Value, "yyyyMMdd", CultureInfo.InvariantCulture);
            aedoReport.ReportRetry.SubmitDT = DateTime.ParseExact(item.Element("submit_date").Value + " " + item.Element("submit_time").Value, "yyyyMMdd HH:mm:ss", CultureInfo.InvariantCulture);
