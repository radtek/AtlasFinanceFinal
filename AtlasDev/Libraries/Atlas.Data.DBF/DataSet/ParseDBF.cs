 ...
 *  Description:
 *  ------------------
 *    Fast DBASE to DataTable parser
 * 
 * 
 *  Author:
 *  ------------------
 *     http://www.codeproject.com/Articles/24247/Load-a-DBF-into-a-DataTable
 *     Updates and corrections by Keith Blows
 *     
 * 
 * 
 *  Revision history: 
 *  ------------------ 
 *     2012-05-20 - Obtained from the web
 *     2012-06-19 - Fixed bug where negative numbers would be imported as zeros
 *     2012-12-07 - Added header check to avoid memory overflow with corrupted DBF
 *     2013-01-02 - Better Decimal.TryParse to ignore unusual PC regional settings
 *     
 * ----------------------------------------------------------------------------------------------------------------- */
...
namespace Atlas.Data.DBF.DataSet
{ 
  ...
  public class ParseDBF
  {
    ...
    public static DataTable ReadDBF(string dbfFile)
    {
      ...
      try
      {
        ...
        // Check header- less than 1,000,000 records and file size is larger than [record size] * [record count]
        if (header.numRecords > 1000000 ||
          header.version > 3 || header.encryptionFlag != 0 ||
          header.updateYear < 12 || header.updateMonth > 12 || header.updateDay > 31 ||
          header.recordLen > 19600 ||
          fi.Length < (header.recordLen * header.numRecords))
        {
          throw new Exception("Invalid dBASE file");
        }
        ...
        for (int counter = 0; counter <= header.numRecords - 1; counter++)
        {
          ...
          foreach (FieldDescriptor field in fields)
          {
            ...
            {
              case 'N':  // Number
                number = Encoding.ASCII.GetString(recReader.ReadBytes(field.fieldLen)).Trim();
                if (number.IndexOf(".") > -1)
                {
                  decimal value;
                ...
                {
                  int value;
                  if (int.TryParse(number, NumberStyles.Any, CultureInfo.InvariantCulture, out value))
                  {
                    row[fieldIndex] = value;
                  }
                  else
              ...
              case 'C': // String
                row[fieldIndex] = Encoding.ASCII.GetString(recReader.ReadBytes(field.fieldLen));//.TrimEnd();
                break;
              ...
              case 'L': // Boolean (Y/N)
                row[fieldIndex] = recReader.ReadByte() == 'Y';
              ...
              case 'F':
                number = Encoding.ASCII.GetString(recReader.ReadBytes(field.fieldLen));
                double dValue;
                if (double.TryParse(number, out dValue))
                {
                  row[fieldIndex] = dValue;
                }
                else
                {
                  row[fieldIndex] = 0.0F;
