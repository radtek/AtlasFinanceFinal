...
namespace Falcon.DuckHawk.Jobs.QuartzTasks.ASS
{
  ...
  [JobName("AssCiReportEmailer")]
  [TriggerName("AssCiReportEmailer")]
  ...
  public class AssCiReportEmailer : IAssCiReportEmailerJob
  {
    ...
    public AssCiReportEmailer(IServiceBus bus, ILifetimeScope scope, ILogger logger, ICiReportService ciReportService)
    {
      _bus = bus;
      _scope = scope;
      _logger = logger;
    ...
    public void Execute(IJobExecutionContext context)
    {
      using (_scope.BeginLifetimeScope())
      {
        try
        {
          _logger.Information("Started Job [AssCiReportEmailer]");
            ...
            for (var i = 0; i <= allowedRegions.Length; i++)
            {
              ...
              var branches =
                new XPQuery<BRN_Branch>(uow).Where(
                  b =>
                    b.BranchId > 1 && !notAllowedBranches.Contains(b.BranchId) &&
          ...
          _bus.Publish(new EmailNotifyMessage(CombGuid.Generate())
          {
            ...
            //To = toEmailGroup
            To = ConfigurationManager.AppSettings["cidistributionlist"] ?? "lee@atcorp.co.za"
          });

          _logger.Information("Finished Job [AssCiReportEmailer]");

        }
        catch (Exception ex)
        {
          _logger.Error(string.Format("Error in Job [AssCiReportEmailer]: {0} - {1}", ex.Message, ex.StackTrace));
        }
      }
    }
  }
