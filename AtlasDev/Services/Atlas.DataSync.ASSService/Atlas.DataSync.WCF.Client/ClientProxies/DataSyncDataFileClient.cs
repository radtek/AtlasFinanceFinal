using System;
using System.ServiceModel;

using Atlas.DataSync.WCF.Interface;


namespace Atlas.DataSync.WCF.Client.ClientProxies
{
  public class DataSyncDataFileClient : IDataFileServer, IDisposable
  {
    #region Public constructor

    public DataSyncDataFileClient(
      TimeSpan? openTimeout = null,  // The interval of time provided for an open operation to complete including security handshakes
      TimeSpan? sendTimeout = null)  // The interval of time provided for an entire operation to complete. This includes both sending of message and receiving reply! The default is 00:01:00.
    {
      var binding = new NetTcpBinding
        {
          CloseTimeout = TimeSpan.FromMinutes(1),                   // The interval of time provided for a close operation to complete. The default is 00:01:00.
          OpenTimeout = openTimeout ?? TimeSpan.FromSeconds(10),
          SendTimeout = sendTimeout ?? TimeSpan.FromSeconds(180),
          TransferMode = TransferMode.Buffered,
          MaxReceivedMessageSize = 500000,
          MaxBufferSize = 500000
        };

      binding.ReaderQuotas.MaxArrayLength = 500000;
      binding.Security.Mode = SecurityMode.None;

      _factory = new ChannelFactory<IDataFileServer>(binding, new EndpointAddress(Config.DATASYNC_SERVER_ADDRESS + "/LMSData/Sync/DataFile/Files"));
      _channel = _factory.CreateChannel();
    }

    #endregion


    #region Public methods

    public long GetFileSize(SourceRequest sourceRequest, string fileName)
    {
      return _channel.GetFileSize(sourceRequest, fileName);
    }


    public bool AppendFileChunk(SourceRequest sourceRequest, string fileName, byte[] buffer)
    {
      return _channel.AppendFileChunk(sourceRequest, fileName, buffer);
    }


    public byte[] DownloadFileChunk(SourceRequest sourceRequest, string fileName, long offset, int chunkSize)
    {
      return _channel.DownloadFileChunk(sourceRequest, fileName, offset, chunkSize);
    }


    public string GetFileChecksum(SourceRequest sourceRequest, string fileName, string method)
    {
      return _channel.GetFileChecksum(sourceRequest, fileName, method);
    }

    #endregion


    #region IDisposable implementation

    /// <summary>
    /// IDisposable.Dispose implementation, calls Dispose(true).
    /// </summary>
    void IDisposable.Dispose()
    {
      Dispose(true);
    }

    /// <summary>
    /// Dispose worker method. Handles graceful shutdown of the
    /// client even if it is an faulted state.
    /// </summary>
    /// <param name="disposing">Are we disposing (alternative
    /// is to be finalizing)</param>
    protected virtual void Dispose(bool disposing)
    {
      if (disposing)
      {
        try
        {
          if (_factory.State != CommunicationState.Faulted)
          {
            _factory.Close();
          }
        }
        finally
        {
          if (_factory.State != CommunicationState.Closed)
          {
            _factory.Abort();
          }
        }
      }
    }

    /// <summary>
    /// Finalizer.
    /// </summary>
    ~DataSyncDataFileClient()
    {
      Dispose(false);
    }

    #endregion


    #region Private fields

    private readonly ChannelFactory<IDataFileServer> _factory;
    private readonly IDataFileServer _channel = null;

    #endregion



  }
}
