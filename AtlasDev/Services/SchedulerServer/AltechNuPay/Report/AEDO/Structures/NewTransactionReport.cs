...
namespace SchedulerServer.AltechNuPay.Report.AEDO.Structures
{
  public class NewTransactionReport
  {
    ...
    public NewTransactionReport(XElement item)
    {
      ...
      aedoReport.ReportNewTransaction.SubmitDT = DateTime.ParseExact(string.Format("{0} {1}", item.Element("submit_date").Value, item.Element("submit_time").Value), "yyyyMMdd HH:mm:ss", CultureInfo.InvariantCulture);
