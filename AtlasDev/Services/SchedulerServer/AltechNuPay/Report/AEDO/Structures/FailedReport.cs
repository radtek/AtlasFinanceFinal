...
namespace SchedulerServer.AltechNuPay.Report.AEDO.Structures
{
  public class FailedReport
  {
    ...
    public FailedReport(XElement item)
    {
      ...
      aedoReport.ReportFailed.FailDT = DateTime.ParseExact(string.Format("{0} {1}", item.Element("fail_date").Value, item.Element("fail_time").Value), "yyyyMMdd HH:mm:ss", CultureInfo.InvariantCulture);
