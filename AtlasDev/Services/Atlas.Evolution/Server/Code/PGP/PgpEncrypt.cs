...
namespace Atlas.Evolution.Server.Code.PGP
{
  ...
  internal class PgpEncrypt
  {
    ...
    public static bool EncryptFile(ILogging log, string publicKeyFilePath, string inputFile, string outputFile)
    {
      var publicKeys = LoadPublicKeys(log, publicKeyFilePath);
      if (publicKeys.Count == 0)
      {
        log.Error("Failed to locate any public keys in {Directory}", publicKeyFilePath);
        return false;
      }
      ...
      try
      {
        ...
        {
          successful = EncryptFileForMultipleReceipients(log, outputStream, inputFile, publicKeys, true, true);
          outputStream.Flush();
          outputStream.Close();
        }
      }
    ...
    /// <returns></returns>
    private static bool EncryptFileForMultipleReceipients(ILogging log, Stream outputStream, string fileName, 
      List<PgpPublicKey> encKey, bool armor = true, bool withIntegrityCheck = true)
    {
      ...
      {
        log.Error(ex, "EncryptFileForMultipleReceipients");
        return false;
      }
    }
    ...
    /// <returns></returns>
    private static List<PgpPublicKey> LoadPublicKeys(ILogging log, string publicKeyFilePath)
    {
