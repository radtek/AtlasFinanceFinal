...
if (!DevExpress.MOD_WIDGETS_WEB) {
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxColorBox", ui, ui.dxDropDownEditor.inherit({
            _supportedKeys: function() {
                ...
                var preventColorUpdate = function(e, fn) {
                ...
                return $.extend(this.callBase(), {
                        ...
                        upArrow: function(e) {
                            return preventColorUpdate.call(this, e, this.close)
                        },
                        downArrow: function(e) {
                            return preventColorUpdate.call(this, e, this._validatedOpening)
                        }
                    })
            },
            _setDefaultOptions: function() {
        ...
        DX.registerComponent("dxTreeView", ui, ui.CollectionWidget.inherit({
            _supportedKeys: function(e) {
                ...
                var select = function(e) {
                        e.preventDefault();
                        this._changeCheckBoxState(this.option("focusedItem"))
                    };
                var expandAllItems = function(e) {
                        if (!this.option("expandAllEnabled"))
                            return;
                        e.preventDefault();
                        var $rootElement = this.option("focusedItem");
                        if (!$rootElement)
                            return;
            ...
            _expandAllItems: function(items, expandedItems) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    var $node = that.element().find("[" + DATA_ITEM_ID + "='" + this._getItemKey(item) + "']");
                    that._fireExpandedStateUpdatedEvt(true, item, $node);
                    that._commonExpandedSetter(item, $node, true);
                    if (item.items)
                        that._expandAllItems(item.items, expandedItems)
                }
            },
            _collapseAllItems: function(items, collapsedItems) {
                ...
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    var $node = that.element().find("[" + DATA_ITEM_ID + "='" + this._getItemKey(item) + "']");
                    that._fireExpandedStateUpdatedEvt(false, item, $node);
                    that._commonExpandedSetter(item, $node, false);
                    collapsedItems.push(item);
                    if (item.items)
                        that._collapseAllItems(item.items, collapsedItems)
                }
            },
            ...
            _optionChanged: function(args) {
                ...
                switch (name) {
                    ...
                    case"expandedItems":
                        ...
                        if (isFocused) {
                            this.element().focus();
                            var key = this.option("focusedItem").attr(DATA_ITEM_ID);
                            var $node = this.element().find("[" + DATA_ITEM_ID + "='" + key + "']");
                            this.option("focusedItem", $node)
                        }
                        break;
            ...
            _updateItemSelection: function(value, itemElement, suppressOnSelectionChanged, $node) {
                ...
                if (currentState === value)
                    return;
                if (!$node) {
                    var $tmpNode = this.element().find("[" + DATA_ITEM_ID + "='" + key + "']");
                    if ($tmpNode.length)
                        $node = $tmpNode
                }
                if ($node)
            ...
            _focusInHandler: function(e) {
                ...
                if (!this.option("focusedItem")) {
                    ...
                    this.option("focusedItem", $activeItem.closest("." + NODE_CLASS))
                }
                else
            ...
            _moveFocus: function(location, e) {
                ...
                switch (location) {
                    ...
                    case FOCUS_FIRST:
                        ...
                        this.option("focusedItem", $firstItem);
                        break;
                    case FOCUS_LAST:
                        var $lastItem = $items.last();
                        if (e.shiftKey)
                            this._updateSelectionToLastItem($items, $items.index(this._nextItem($items)));
                        this.option("focusedItem", $lastItem);
                        break;
                    case FOCUS_RIGHT:
            ...
            _expandFocusedContainer: function() {
                if (!this.option("focusedItem"))
                    return;
                var $focusedItem = this.option("focusedItem");
                if ($focusedItem.hasClass(IS_LEAF))
                    return;
                var $node = $focusedItem.find("." + NODE_CONTAINER_CLASS).eq(0);
                if ($node.hasClass(OPENED_NODE_CONTAINER_CLASS)) {
                    this.option("focusedItem", this._nextItem(this._nodeElements()));
                    return
                }
    ...
    (function($, DX) {
        ...
        dataGrid.EditorFactoryController = dataGrid.ViewController.inherit(function() {
            ...
            return {
                    ...
                    init: function() {
                        this.createAction("onEditorPreparing", {excludeValidators: ['designMode', 'disabled']});
                        this.createAction("onEditorPrepared", {excludeValidators: ['designMode', 'disabled']});
                    ...
                    _attachContainerEventHandlers: function() {
                        ...
                        if ($container) {
                            ...
                            isIE10OrLower && $container.on('dxclick dxpointerup dxpointerdown', '.' + DATAGRID_FOCUS_OVERLAY_CLASS, $.proxy(that._focusOverlayEventProxy, that))
                        }
                    },
                    _focusOverlayEventProxy: function(e) {
                        ...
                        this._$focusedElement && this._$focusedElement.find('input').focus()
                    },
                    dispose: function() {
                ...
                if (that._tableElement) {
                    ...
                    $.each(cells, function(index, item) {
                        if (item.getBoundingClientRect) {
                            ...
                            width = Math.ceil(clientRect.right - clientRect.left)
                        }
                        else
            ...
            extenders: {
                ...
                views: {rowsView: {
                        updateFreeSpaceRowHeight: function() {
                            ...
                            that.callBase();
                            if ($tooltipContent.length && $rowElements.length === 1 && $freeSpaceRowElement && (!$freeSpaceRowElement.is(":visible") || $tooltipContent.outerHeight() > $freeSpaceRowElement.outerHeight())) {
                ...
                _updateFilterOperationChooser: function($menu, column, $editorContainer) {
                    ...
                    $menu.dxMenu({
                        ...
                        onSubmenuHiding: function() {
                            $menu.blur();
                            $menu.dxMenu("instance").option("focusedItem", null);
    ...
    (function($, DX) {
        ...
        dataGrid.RowsView = dataGrid.ColumnsView.inherit({
            ...
            _createTable: function() {
                var that = this;
                return that.callBase.apply(that, arguments).on("dxclick", '.dx-row', that.createAction(function(args) {
                        var e = args.jQueryEvent;
    ...
    (function($, DX) {
        ...
        dataGrid.ErrorHandlingController = dataGrid.ViewController.inherit({
            ...
            renderErrorRow: function(message, rowIndex) {
                ...
                $("<td/>").attr("colspan", that._columnHeadersView.getHeadersLength()).prepend($closeButton).append($errorMessage).appendTo($errorRow)
            },
