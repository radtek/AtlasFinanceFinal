...
if (!DevExpress.MOD_VIZ_GAUGES) {
    ...
    (function(DX, $, undefined) {
        ...
        viz.gauges.__internals.BaseScale = DX.Class.inherit({
            ctor: function(parameters) {
                var that = this,
                    renderer;
                that._renderer = renderer = parameters.renderer;
                that._container = parameters.container;
                that._translator = parameters.translator;
                that._root = renderer.g().attr({'class': 'dxg-scale'});
                that._majorTicks = renderer.g().attr({'class': 'dxg-major-ticks'});
                that._minorTicks = renderer.g().attr({'class': 'dxg-minor-ticks'});
                that._labels = renderer.g().attr({'class': 'dxg-labels'})
            },
            dispose: function() {
                var that = this;
        ...
        viz.gauges.__internals.Title = DX.Class.inherit({
            ...
            render: function(titleOptions, subtitleOptions) {
                ...
                if (!hasTitle && !hasSubtitle)
                    return that;
                that._root = renderer.g().attr({'class': 'dxg-title'}).append(that._container);
                title = hasTitle ? renderer.text(titleOptions.text, 0, 0).attr({align: 'center'}).css(core.utils.patchFontOptions(titleOptions.font)).append(that._root) : null;
                subtitle = hasSubtitle ? renderer.text(subtitleOptions.text, 0, 0).attr({align: 'center'}).css(core.utils.patchFontOptions(subtitleOptions.font)).append(that._root) : null;
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.gauges.dxBaseGauge = DX.viz.core.BaseWidget.inherit({
            ...
            _disposeCore: function() {
                ...
                that._root = that._translator = that._themeManager = that._tracker = that._layoutManager = that._title = that._tooltip = null
            },
            _refresh: function() {
                var that = this,
    ...
    (function(DX, $, undefined) {
        ...
        DX.viz.gauges.dxGauge = DX.viz.gauges.dxBaseGauge.inherit({
            ...
            _prepareSubvalueIndicators: function() {
                ...
                isRecreate = that._subvalueIndicatorSettings.type !== that._subvalueIndicatorType;
        ...
        DX.registerComponent("dxCircularGauge", DX.viz.gauges, DX.viz.gauges.dxGauge.inherit({
            ...
            _setupCodomain: function() {
                var that = this,
                    ...
                    sides;
                startAngle = _isFinite(startAngle) ? normalizeAngle(startAngle) : 225;
                endAngle = _isFinite(endAngle) ? normalizeAngle(endAngle) : -45;
            ...
            _measureMainElements: function(elements) {
                ...
                _each(elements, function(_, element) {
                    var bounds = element.measure({radius: that._area.radius - (_Number(element._options.offset) || 0)});
            ...
            _updateElementPosition: function(element) {
                element.resize({
                    x: this._area.x,
                    y: this._area.y,
                    radius: round(this._area.radius - (_Number(element._options.offset) || 0))
                })
            },
            ...
            _getApproximateScreenRange: function() {
                var that = this,
                    area = that._area,
                    r = min(that._canvas.width / (area.sides.right - area.sides.left), that._canvas.height / (area.sides.down - area.sides.up));
    ...
    (function(DX, $, undefined) {
        ...
        DX.registerComponent("dxLinearGauge", DX.viz.gauges, DX.viz.gauges.dxGauge.inherit({
            ...
            _updateElementPosition: function(element) {
                var offset = _Number(element._options.offset) || 0;
                element.resize({
                    x: round(this._area.x + offset),
                    y: round(this._area.y + offset)
                })
            },
            _createScale: function(parameters) {
        ...
        var dxBarGauge = gauges.dxBaseGauge.inherit({
                ...
                _renderContent: function() {
                    ...
                    if (context.textEnabled) {
                        ...
                        context.fontStyles = core.utils.patchFontOptions(_extend({}, that._themeManager.theme().label.font, labelOptions.font, {color: null}));
        ...
        _extend(BarWrapper.prototype, {
            ...
            beginAnimation: function(value) {
                var that = this,
                    angle;
                that._value = value;
                angle = that._context.translator.translate(value);
