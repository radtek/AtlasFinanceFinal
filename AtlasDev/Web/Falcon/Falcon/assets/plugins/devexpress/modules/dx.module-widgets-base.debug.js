...
if (!DevExpress.MOD_WIDGETS_BASE) {
    ...
    (function($, DX, undefined) {
        ...
        var onActiveElementChange = function(callback) {
                ui.dxScrollable.__internals.focusChangeCallback = callback;
                if (window.addEventListener)
                    window.addEventListener('focus', callback, true);
                else
                    window.attachEvent('onfocusin', callback)
            },
            offActiveElementChange = function(callback) {
                if (window.addEventListener)
                    window.removeEventListener('focus', callback);
                else
                    window.detachEvent('onfocusin', callback)
            };
        ...
        ui.dxScrollable.SimulatedStrategy = DX.Class.inherit({
            ...
            _init: function(scrollable) {
                ...
                onActiveElementChange(this._proxiedActiveElementChangeHandler)
            },
            _activeElementChangeHandler: function(e) {
                var activeElement = e instanceof Event ? document.activeElement : e;
                if (this._activeElement !== activeElement && !$(activeElement).is("." + SCROLLABLE_CONTAINER_CLASS + ", ." + SCROLLABLE_CONTENT_CLASS))
            ...
            dispose: function() {
                offActiveElementChange(this._proxiedActiveElementChangeHandler);
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxBox", ui, ui.CollectionWidget.inherit({
            ...
            _postprocessRenderItem: function(args) {
                ...
                boxConfig && this._createComponent(args.itemContent, "dxBox", boxConfig)
            },
            _visibilityChanged: function(visible) {
                if (visible)
                    this._dimensionChanged()
        ...
        DX.registerComponent("dxDropDownList", ui, ui.dxDropDownEditor.inherit({
            ...
            _setFocusPolicy: function() {
                if (!this.option("focusStateEnabled") || !this._list)
                    return;
                this._list.option("focusedItem", null)
            },
            ...
            _processDataSourceChanging: function() {
                this._setListDataSource();
                this._loadValue(this.option("value")).fail($.proxy(this.reset, this))
            },
            reset: function() {
            ...
            _hasItemsToShow: function() {
                ...
                return isMinFilterLengthExceeded && resultAmount && !foundSingleResultEqualToValue && this.element().hasClass("dx-state-focused")
            },
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxNumberBox", ui, ui.dxTextEditor.inherit({
            ...
            _inputInvalidHandler: function() {
                var $input = this._input();
                this.option("value", DX.utils.isDefined(this._oldValue) ? value : null);
                this._oldValue = null
            },
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxTabs", ui, ui.CollectionWidget.inherit({
            ...
            _createNavButton: function(offset) {
                return this._createComponent($("<div>").addClass(TABS_NAV_BUTTON_CLASS), "dxButton", {
                        focusStateEnabled: false,
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxList", ui, ui.CollectionWidget.inherit({
            ...
            _moveFocus: function(location) {
                this.callBase.apply(this, arguments);
                this.scrollToItem(this.option("focusedItem"))
            },
            _optionChanged: function(args) {
                switch (args.name) {
                    case"showNextButton":
        ...
        DX.registerComponent("dxTileView", ui, ui.CollectionWidget.inherit({
            ...
            _moveFocus: function(location) {
                ...
                var cells = this.cells,
                    index = this.option("focusedItem").index(),
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxGallery", ui, ui.CollectionWidget.inherit({
            ...
            _setFocusOnSelect: function() {
                ...
                this.option("focusedItem", selectedItem);
                this._userInteraction = false
            },
            ...
            _moveFocus: function(location) {
                this.callBase.apply(this, arguments);
                var index = this.itemElements().index(this.option("focusedItem"));
        ...
        DX.registerComponent("dxCalendar", ui, ui.Editor.inherit({
            ...
            _isDateInInvalidRange: function() {
                if (this._isBoundary(this._newDate))
                    return;
                var normalizedDate = utils.normalizeDate(this._newDate, this._minDate, this._maxDate);
                return normalizedDate === this._minDate || normalizedDate === this._maxDate
            },
            _isBoundary: function(date) {
                if (utils.isDefined(this.option("min"))) {
                    this._minDate = this.option("min");
                    var minMonth = this._minDate.getMonth(),
                        minYear = this._minDate.getYear()
                }
                if (utils.isDefined(this.option("max"))) {
                    this._maxDate = this.option("max");
                    var maxMonth = this._maxDate.getMonth(),
                        maxYear = this._maxDate.getYear()
                }
                if (utils.isDefined(date))
                    var dateMonth = date.getMonth(),
                        dateYear = date.getYear();
                return dateMonth === minMonth && dateYear === minYear || dateMonth === maxMonth && dateYear === maxYear
            },
            _trimUnusedViews: function(viewsArray) {
                var animatedViews = viewsArray.slice(),
                    indexCurrentView = $.inArray(this._view, animatedViews),
            ...
            _moveInRange: function(arrayViews, swipePosition) {
                var length = arrayViews.length,
                    summaryWidth = 0;
                for (var i = length - 1; i >= 0; i--) {
                    DX.translator.move(arrayViews[i]._$table, {left: summaryWidth * swipePosition});
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxTimeView", ui, ui.Editor.inherit({
            ...
            _renderField: function($content) {
                $content.addClass(TIMEVIEW_FIELD_CLASS);
                this._createComponent($content, "dxBox", {
                    direction: "row",
                    align: "center",
                    crossAlign: "center",
                    items: [{
    ...
    (function($, DX) {
        ...
        DX.registerComponent("dxDateView", ui, ui.Editor.inherit({
            ...
            _render: function() {
                this._rollersShouldBeRefreshed = true;
                this.callBase();
            ...
            _refreshRollers: function() {
                ...
                $.each(this._rollers, function(type) {
                    var correctIndex = that._rollerConfigs[type].getIndex(that.option("value"));
                    this.update();
            ...
            _setRollerState: function(roller, selectedIndex) {
                if (selectedIndex !== roller.selectedIndex) {
                    var rollerValue = roller.valueItems[selectedIndex],
                        ...
                        currentValue = new Date(this.option("value")),
                    ...
                    this._rollersShouldBeRefreshed = false;
            ...
            _optionChanged: function(args) {
                switch (args.name) {
                    ...
                    case"value":
                        ...
                        this._rollersShouldBeRefreshed && this._renderRollers();
                        break;
                    default:
                        this.callBase(args)
                }
        ...
        DX.registerComponent("dxDateBox", ui, ui.dxDropDownEditor.inherit({
            ...
            _valueChangeEventHandler: function(e) {
                var text = this.option("text"),
                    ...
                    isValid = !(!date && text && text.length) && utils.dateInRange(date, this.option("min"), this.option("max"));
                this.option({
                    isValid: isValid,
            ...
            renderPopupContent: function() {
                ...
                $("<div>").appendTo($popupContent).dxBox({
                    ...
                    itemTemplate: $.proxy(function(data, _, $container) {
                        ...
                        switch (data.name) {
                            case"calendar":
                                $element = this._widget.element();
                                break;
                            case"time":
                                $element = this._timeView.element();
                                break
                        }
        ...
        DX.registerComponent("dxLookup", ui, ui.dxDropDownList.inherit({
            ...
            _popupShowingHandler: function() {
                ...
                if (this.option("cleanSearchOnOpening")) {
                    ...
                    this._list && this._list.option("focusedItem", null)
                }
            },
            _renderPopup: function() {
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxAutocomplete", ui, ui.dxDropDownList.inherit({
            _supportedKeys: function() {
                var item = this._list ? this._list.option("focusedItem") : null;
                return $.extend(this.callBase(), {
                        upArrow: function(e) {
                            e.preventDefault();
                            e.stopPropagation();
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxTabPanel", ui, ui.dxMultiView.inherit({
            ...
            _boxItemTemplate: function(data, _, $content) {
                if (data.name === "tabs") {
                    this._$tabs = $("<div>");
                    this._createComponent(this._$tabs, "dxTabs", this._tabConfig());
